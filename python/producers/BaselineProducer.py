...
class BaselineProducer(Module):
    ...
    def analyze(self, event):
        ...
        if not (len(event.selectedElectrons) == 2 and len(event.selectedMuons) == 0):
            return False
        # if not (
    ...
    def _select_Z_candidates(self, event):
        ...
        for el1, el2 in electron_pairs:
            ...
            if not ((el1.pt > 33 and el2.pt > 20) or (el1.pt > 20 and el2.pt > 33)):
                continue
    ...
    def _select_muons(self, event):
        ...
        for mu in muons:
            ...
            if mu.pt > 10 and abs(mu.eta) < 2.4 and mu.tightId == True and pfRelIso < 0.15: # and abs(mu.dxy) < 0.5 and abs(mu.dz) < 1 and mu.pfRelIso03_all < 0.15 and mu.tightId == True:
                mu._wp_ID = 'TightID'
    ...
    def _select_electrons(self, event):
        ...
        for el in electrons:
            ...
            if el.pt > 10 and abs(el.eta) < 2.4 and el.mvaIso_WP80 == True: #and abs(el.dxy) < 0.5 and abs(el.dz) < 1 and el.mvaIso_WP80 == True:
                el._wp_ID = 'wp80iso'
