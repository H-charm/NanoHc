...
class EventProducer(Module):
    ...
    def analyze(self, event):
        ...
        if len(event.selectedMuons) == 2 and len(event.selectedElectrons) == 0:
            self._select_Zmu_candidates(event)
        elif len(event.selectedElectrons) == 2 and len(event.selectedMuons) == 0:
            self._select_Zel_candidates(event)
        else:
            return False
    ...
    def _select_triggers(self, event):
        ...
        if self.isMC or self.sample == "any":
            passTrigger = passSingleEle or passSingleMu
        else:
            if self.sample == "" : sys.exit("ERROR: sample must be set in data")
    ...
    def _select_Zmu_candidates(self, event):
        ...
        for mu1, mu2 in muon_pairs:
            if mu1.pdgId + mu2.pdgId != 0: # Select only opposite-sign muons
                continue
    ...
    def _select_jets(self, event):
        ...
        for jet in jets:
            if jet.pt <= 20 or abs(jet.eta) >= 2.5 or jet.jetId !=6:
                continue
